...
bool editTitle(char* newTitle, char* fileName);
bool editDesc(char* newDesc, char* fileName);
...
char* SVGtoJSON(char* fileName)
{
  if (fileName == NULL)
    return NULL;

  const SVGimage* imge = createSVGimage(fileName);

  if (!validateSVGimage((SVGimage*)imge, "parser/svg.xsd"))
    return NULL;
  ...
  svgStr = malloc(sizeof(char) * (69 + 16));
  sprintf(svgStr, "{\"fileName\":\"%s\",\"numRect\":%d,\"numCirc\":%d,\"numPaths\":%d,\"numGroups\":%d}", fName, getLength(recList), getLength(cirList), getLength(pthList), getLength(grpList));
...
char* circListToJSON(char* fileName)
{
  SVGimage *img = createSVGimage(fileName);
  const List *list = img->circles;
...
char* rectListToJSON(char *fileName)
{

  SVGimage *img = createSVGimage(fileName);
  const List *list = img->rectangles;
...
char* pathListToJSON(char *fileName)
{

  SVGimage *img = createSVGimage(fileName);
  const List *list = img->paths;
...
char* groupListToJSON(char *fileName)
{

  SVGimage *img = createSVGimage(fileName);
  const List *list = img->groups;
...
char* getDesc(char* fileName) {
  ...
  SVGimage *imge = createSVGimage(fileName);

  char *descJSON;

  removeChar(imge->title, '\n');
  removeChar(imge->title, '\"');
  removeChar(imge->description, '\n');
  removeChar(imge->description, '\"');

  int len = strlen(imge->title) + strlen(imge->description);
  descJSON = malloc(sizeof(char) * (29 + len + 1));
  sprintf(descJSON, "{\"title\":\"%s\",\"description\":\"%s\"}", imge->title, imge->description);
...
bool validate(char* fileName) 
{
  ...
  if (createValidSVGimage(fileName, "./parser/svg.xsd")) {
    return true;
  } else {
    return false;
  }
...
bool editTitle(char* newTitle, char* fileName) {
  ...
  SVGimage *img = createSVGimage(fileName);
  strncpy(img->title, newTitle, 254);
...
bool editDesc(char* newDesc, char* fileName) {
  ...
  SVGimage *img = createSVGimage(fileName);
  strncpy(img->description, newDesc, 254);
